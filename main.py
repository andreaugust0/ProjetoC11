...
ds['Taxa de Cumprimento (%)'] = (ds['Total bilateral allocations($ billion)'] / ds['Total bilateral commitments($ billion)']) * 100

ds['Taxa de Cumprimento (%)'] = ds['Taxa de Cumprimento (%)'].fillna(100)

colunas_relatorio = ['Country', 'Total bilateral allocations($ billion)', 'Total bilateral commitments($ billion)', 'Taxa de Cumprimento (%)']

top_confiaveis = ds.sort_values(by='Taxa de Cumprimento (%)', ascending=False)
top_menos_confiaveis = ds.sort_values(by='Taxa de Cumprimento (%)', ascending=True)
df_plot_controversos = top_menos_confiaveis.dropna().head(10)

plt.figure(figsize=(10, 7))

bars = plt.barh(df_plot_controversos['Country'], df_plot_controversos['Taxa de Cumprimento (%)'], color='#2E86AB', alpha=0.8, edgecolor='black')
plt.xlabel('Taxa de Cumprimento (%)')
plt.title('Top 10 Países Mais Controversos (Menor Taxa de Cumprimento)')
...
# Gráfico
plt.figure(figsize=(10, 6))
plt.bar([i - bar_width/2 for i in x], eu_values, width=bar_width, label='UE', color='#2E86AB', alpha=0.8, edgecolor='black')
plt.bar([i + bar_width/2 for i in x], neu_values, width=bar_width, label='Não UE', color='#FFD700', alpha=0.8, edgecolor='black')
...
for i in range(len(x)):
    plt.text(i - bar_width/2, eu_values[i] + 0.5, f'${eu_values[i]:.1f}B', ha='center', fontweight='bold')
    plt.text(i + bar_width/2, neu_values[i] + 0.5, f'${neu_values[i]:.1f}B', ha='center', fontweight='bold')
...
bar_financial = plt.bar(x_indexes - width/2, financial, width=width, label='Financial', color='#2E86AB', alpha=0.8, edgecolor='black')
bar_military = plt.bar(x_indexes + width/2, military, width=width, label='Military', color='#FFD700', alpha=0.8, edgecolor='black')

labels_financial = [f'${h:.1f}' if h > 0.1 else '' for h in financial]
labels_military = [f'${h:.1f}' if h > 0.1 else '' for h in military]

plt.bar_label(bar_financial, labels=labels_financial, padding=3, fontweight='bold')
plt.bar_label(bar_military, labels=labels_military, padding=3, fontweight='bold')
